const {
  ...
  const updateOrderStatus = async ({
    ...
    try {
      ...
      if (!status || (status !== 'accepted' && status !== 'rejected' && status !== 'auto-rejected' &&
  status !== 'completed')) {
      ...
      // 🔒 IDEMPOTENTNOST
      // Ako je narudžba već u traženom statusu, ne radi ništa i NE šalji SMS/push.
      // Ovo sprječava dvostruke poruke klijentu kad PATCH stigne više puta
      // (retry frontenda, više instanci, reconnect itd.).
      if (orderData.status === status) {
        console.log(`Order ${orderId} already has status "${status}" – skipping (no SMS/push).`);
        return true;
      }
 
      // 🔒 DOZVOLJENI PRIJELAZI STATUSA
      // Prihvaćanje/odbijanje smiju se raditi samo iz "pending".
      // "completed" ide iz "accepted". Sve ostalo se preskače (bez slanja poruka).
      ...
      if (!allowedTransitions[status]?.includes(orderData.status)) {
        console.log(
          `Skipping status change "${orderData.status}" -> "${status}" for order ${orderId} (not an
  allowed transition, no SMS/push).`
        );
        return true;
      }
      ...
      if (status === "accepted") {
        ...
        if (phone && price != null) {
          ...
          if (result.committed) {
            console.log(
              `Loyalty podaci za telefon ${phone} uspješno ažurirani. Novi bodovi:
  ${result.snapshot.val().points}`
            );
          }
        }
      }
      ...
      if (status === 'accepted') {
        ...
        if (coupon) {
          message = lang === 'hr'
            ...
            : 'Unfortunately your order has been rejected, but a 10€ coupon has been returned to your
  account.';
        } else {
          message = lang === 'hr'
        ...
        if (coupon) {
          message = lang === 'hr'
            ? 'Nažalost vaša narudžba je automatski odbijena zbog prevelikog obujma posla, pokušajte
  kasnije. Kupon od 10€ je vraćen na Vaš račun.'
            : 'Unfortunately your order was automatically rejected due to high demand. Please try
  again later. A 10€ coupon has been returned to your account.';
        } else {
          message = lang === 'hr'
            ? 'Nažalost vaša narudžba je automatski odbijena zbog prevelikog obujma posla, pokušajte
  kasnije.'
